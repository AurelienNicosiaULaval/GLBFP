...
LBFP <- function(
  ...
  if (
    ...
    stop(
      "Inputs must be numeric vectors for x and b, and a matrix or data frame for data."
    )
  }
  d <- ncol(data)
  if (
    ...
    stop(
      "The point, min_vals, max_vals and bin width vectors must match the dataset dimensions."
    )
  }
  if (any(b <= 0)) {
  ...
  # Compute of I(k)
  lowerbound_cell_IK <- sapply(1:d, function(i) {
    a[i] + (idx[i] - 1) * b[i]
  })
  upperbound_cell_IK <- sapply(1:d, function(i) {
    a[i] + idx[i] * b[i]
  })
