...
#' @examples
#'  # Example for GLBFP_estimate function with bivariate normale distribution
#' data <- as.data.frame(mvrnorm(n = 1000, mu =  c(0, 0), Sigma = matrix(c(1, 0.5, 0.5, 1), nrow = 2)))
#'
#' ##  Use of grid_size
#' b <- c(0.5, 0.5)
#' out <- GLBFP_estimate(data, b, grid_size = 30)
#' out
#' plot(out)
#'
#' ## with a specific grid_points
#' grid_points <- data.frame(X = seq(-1,1,0.1), Y =  seq(-1,1,0.1))
#' out <- GLBFP_estimate(data, b, grid_points = grid_points)
#' out
...
GLBFP_estimate <- function(data, b = compute_bi_optim(data, m = rep(1,  ncol(data))), m = rep(1,  ncol(data)), grid_size = 20, grid_points = NULL, min_vals =  apply(data, 2, min), max_vals =  apply(data, 2, max) ) {
  ...
  estimations <- sapply(1:nrow(grid_points), function(i) {
    ...
    GLBFP(point, data, b, m, min_vals, max_vals)$estimation
  })
