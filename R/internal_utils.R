...
glbfp_validate_data <- function(data, allow_na = FALSE) {
  ...
  if (is.data.frame(data)) {
    ...
    if (!all(numeric_cols)) {
      stop("`data` must contain only numeric columns.", call. = FALSE)
    }
...
glbfp_validate_bounds <- function(min_vals, max_vals, d) {
  ...
  if (any(min_vals >= max_vals)) {
    stop(
      "Each entry in `min_vals` must be strictly smaller than `max_vals`; constant data require explicit non-degenerate bounds.",
      call. = FALSE
    )
  }
...
glbfp_prepare_grid <- function(data, grid_size = 20, grid_points = NULL, min_vals, max_vals) {
  ...
  if (!is.null(grid_points)) {
    ...
    if (is.data.frame(grid_points)) {
      numeric_cols <- vapply(grid_points, is.numeric, logical(1))
      if (!all(numeric_cols)) {
        stop("`grid_points` must contain only numeric columns.", call. = FALSE)
      }
    } else if (!is.numeric(grid_points)) {
      stop("`grid_points` must be numeric.", call. = FALSE)
    }
    ...
    if (
      !is.numeric(grid_size) ||
        ...
        !is.finite(grid_size) ||
        grid_size <= 1 ||
        abs(grid_size - round(grid_size)) > sqrt(.Machine$double.eps)
    ) {
...
glbfp_nearest_grid_prediction <- function(grid, densities, newdata) {
  ...
  if (is.data.frame(newdata)) {
    numeric_cols <- vapply(newdata, is.numeric, logical(1))
    if (!all(numeric_cols)) {
      stop("`newdata` must contain only numeric columns.", call. = FALSE)
    }
