...
test_that("grid_size must be a single integer greater than one", {
  set.seed(9)
  data <- cbind(rnorm(80), rnorm(80))

  expect_error(LBFP_estimate(data, b = c(0.9, 0.9), grid_size = 1), "integer greater than 1")
  expect_error(LBFP_estimate(data, b = c(0.9, 0.9), grid_size = c(5, 6)), "integer greater than 1")
  expect_error(LBFP_estimate(data, b = c(0.9, 0.9), grid_size = 5.5), "integer greater than 1")
})
